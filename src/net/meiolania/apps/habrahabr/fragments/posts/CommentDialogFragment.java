...
public class CommentDialogFragment extends SherlockDialogFragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		author = getArguments().getString(
				PostsCommentsFragment.EXTRA_COMMENT_AUTHOR);
		comment = getArguments().getString(
				PostsCommentsFragment.EXTRA_COMMENT_BODY);
		score = getArguments().getString(
				PostsCommentsFragment.EXTRA_COMMENT_SCORE);
		time = getArguments().getString(
				PostsCommentsFragment.EXTRA_COMMENT_TIME);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (rating > 0)
			cScore.setTextColor(activity.getResources().getColor(
		...
		cBody.loadDataWithBaseURL(null, STYLESHEET + comment, "text/html",
				"UTF-8", null);
		return v;
	}
